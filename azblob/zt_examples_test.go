...
func Example() {
	...
	data := "Hello World!"
	_, err = blobURL.Upload(ctx, strings.NewReader(data), BlobHTTPHeaders{ContentType: "text/plain"}, Metadata{}, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
	if err != nil {
		log.Fatal(err)
	}
...
func ExampleContainerURL_SetContainerAccessPolicy() {
	...
	_, err = blobURL.Upload(ctx, strings.NewReader("Hello World!"), BlobHTTPHeaders{ContentType: "text/plain"},
		Metadata{}, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
	if err != nil {
		log.Fatal(err)
	}
...
func ExampleBlobAccessConditions() {
	...
	upload, err := blobURL.Upload(ctx, strings.NewReader("Text-1"), BlobHTTPHeaders{}, Metadata{}, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
	...
	showResult(blobURL.Download(ctx, 0, 0,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfModifiedSince: upload.LastModified()}}, false, ClientProvidedKeyOptions{}))

	// Download blob content if the blob hasn't been modified in the last 24 hours (fails):
	showResult(blobURL.Download(ctx, 0, 0,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfUnmodifiedSince: time.Now().UTC().Add(time.Hour * -24)}}, false, ClientProvidedKeyOptions{}))
	...
	upload, err = blobURL.Upload(ctx, strings.NewReader("Text-2"), BlobHTTPHeaders{}, Metadata{},
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfMatch: upload.ETag()}}, DefaultAccessTier, ClientProvidedKeyOptions{})
	showResult(upload, err)

	// Download content if it has changed since the version identified by ETag (fails):
	showResult(blobURL.Download(ctx, 0, 0,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfNoneMatch: upload.ETag()}}, false, ClientProvidedKeyOptions{}))
	...
	showResult(blobURL.Upload(ctx, strings.NewReader("Text-3"), BlobHTTPHeaders{}, Metadata{},
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfNoneMatch: ETagAny}}, DefaultAccessTier, ClientProvidedKeyOptions{}))
}
...
func ExampleMetadata_blobs() {
	...
	_, err = blobURL.Upload(ctx, strings.NewReader("Some text"), BlobHTTPHeaders{},
		Metadata{"author": "Jeffrey", "app": creatingApp}, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
	if err != nil {
		log.Fatal(err)
	}
...
func ExampleBlockBlobURL() {
	...
	_, err = blobURL.CommitBlockList(ctx, base64BlockIDs, BlobHTTPHeaders{}, Metadata{}, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
	if err != nil {
		log.Fatal(err)
	}
...
func ExampleAppendBlobURL() {
	...
	ctx := context.Background() // This example uses a never-expiring context
	_, err = appendBlobURL.Create(ctx, BlobHTTPHeaders{}, Metadata{}, BlobAccessConditions{}, ClientProvidedKeyOptions{})
	if err != nil {
		log.Fatal(err)
	}
...
func ExamplePageBlobURL() {
	...
	_, err = blobURL.Create(ctx, PageBlobPageBytes*4, 0, BlobHTTPHeaders{},
		Metadata{}, BlobAccessConditions{}, DefaultPremiumBlobAccessTier, ClientProvidedKeyOptions{})
	if err != nil {
		log.Fatal(err)
	}
...
func Example_blobSnapshots() {
	...
	_, err = baseBlobURL.Upload(ctx, strings.NewReader("Some text"), BlobHTTPHeaders{}, Metadata{}, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
	if err != nil {
		log.Fatal(err)
	}
	...
	_, err = baseBlobURL.Upload(ctx, strings.NewReader("New text"), BlobHTTPHeaders{}, Metadata{}, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
	if err != nil {
		log.Fatal(err)
	}
...
func ExampleListBlobsHierarchy() {
	...
	for _, blobName := range blobNames {
		...
		_, err := blobURL.Upload(ctx, strings.NewReader("test"), BlobHTTPHeaders{}, nil, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
