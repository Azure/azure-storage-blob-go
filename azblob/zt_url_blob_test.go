...
func waitForCopy(c *chk.C, copyBlobURL BlockBlobURL, blobCopyResponse *BlobStartCopyFromURLResponse) {
	...
	for status != CopyStatusSuccess {
		props, _ := copyBlobURL.GetProperties(ctx, BlobAccessConditions{}, nil, nil, nil)
...
func (s *aztestsSuite) TestBlobStartCopyDestEmpty(c *chk.C) {
	...
	resp, err := copyBlobURL.Download(ctx, 0, 20, BlobAccessConditions{}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobStartCopyMetadata(c *chk.C) {
	...
	resp2, err := copyBlobURL.GetProperties(ctx, BlobAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobStartCopyUsingSASSrc(c *chk.C) {
	...
	resp2, err := copyBlobURL.Download(ctx, 0, int64(len(blockBlobDefaultData)), BlobAccessConditions{}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobStartCopySourceIfModifiedSinceTrue(c *chk.C) {
	...
	resp, err := destBlobURL.GetProperties(ctx, BlobAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobStartCopySourceIfMatchTrue(c *chk.C) {
	...
	resp, err := blobURL.GetProperties(ctx, BlobAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	etag := resp.ETag()
	...
	resp2, err := destBlobURL.GetProperties(ctx, BlobAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	c.Assert(resp2.NewMetadata(), chk.DeepEquals, basicMetadata)
}
...
func (s *aztestsSuite) TestBlobStartCopyDestIfMatchTrue(c *chk.C) {
	...
	destBlobURL, _ := createNewBlockBlob(c, containerURL)
	resp, _ := destBlobURL.GetProperties(ctx, BlobAccessConditions{}, nil, nil, nil)
	etag := resp.ETag()
...
func (s *aztestsSuite) TestBlobAbortCopyInProgress(c *chk.C) {
	...
	resp2, _ := copyBlobURL.GetProperties(ctx, BlobAccessConditions{}, nil, nil, nil)
...
func (s *aztestsSuite) TestBlobSnapshotMetadata(c *chk.C) {
	...
	resp2, err := snapshotURL.GetProperties(ctx, BlobAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	c.Assert(resp2.NewMetadata(), chk.DeepEquals, basicMetadata)
}
...
func (s *aztestsSuite) TestBlobDownloadDataNonExistantBlob(c *chk.C) {
	...
	_, err := blobURL.Download(ctx, 0, 0, BlobAccessConditions{}, false, nil, nil, nil)
	validateStorageError(c, err, ServiceCodeBlobNotFound)
}
...
func (s *aztestsSuite) TestBlobDownloadDataNegativeOffset(c *chk.C) {
	...
	_, err := blobURL.Download(ctx, -1, 0, BlobAccessConditions{}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
}
...
func (s *aztestsSuite) TestBlobDownloadDataOffsetOutOfRange(c *chk.C) {
	...
	_, err := blobURL.Download(ctx, int64(len(blockBlobDefaultData)), CountToEnd, BlobAccessConditions{}, false, nil, nil, nil)
...
func (s *aztestsSuite) TestBlobDownloadDataCountNegative(c *chk.C) {
	...
	_, err := blobURL.Download(ctx, 0, -2, BlobAccessConditions{}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
}
...
func (s *aztestsSuite) TestBlobDownloadDataCountZero(c *chk.C) {
	...
	resp, err := blobURL.Download(ctx, 0, 0, BlobAccessConditions{}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobDownloadDataCountExact(c *chk.C) {
	...
	resp, err := blobURL.Download(ctx, 0, int64(len(blockBlobDefaultData)), BlobAccessConditions{}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobDownloadDataCountOutOfRange(c *chk.C) {
	...
	resp, err := blobURL.Download(ctx, 0, int64(len(blockBlobDefaultData))*2, BlobAccessConditions{}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobDownloadDataContentMD5(c *chk.C) {
	...
	resp, err := blobURL.Download(ctx, 10, 3, BlobAccessConditions{}, true, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobDownloadDataIfModifiedSinceTrue(c *chk.C) {
	...
	resp, err := blobURL.Download(ctx, 0, 0,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfModifiedSince: currentTime}}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	c.Assert(resp.ContentLength(), chk.Equals, int64(len(blockBlobDefaultData)))
}
...
func (s *aztestsSuite) TestBlobDownloadDataIfUnmodifiedSinceTrue(c *chk.C) {
	...
	resp, err := blobURL.Download(ctx, 0, 0,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfUnmodifiedSince: currentTime}}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	c.Assert(resp.ContentLength(), chk.Equals, int64(len(blockBlobDefaultData)))
}
...
func (s *aztestsSuite) TestBlobDownloadDataIfMatchTrue(c *chk.C) {
	...
	resp2, err := blobURL.Download(ctx, 0, 0,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfMatch: etag}}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	c.Assert(resp2.ContentLength(), chk.Equals, int64(len(blockBlobDefaultData)))
}
...
func (s *aztestsSuite) TestBlobDownloadDataIfNoneMatchTrue(c *chk.C) {
	...
	resp2, err := blobURL.Download(ctx, 0, 0,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfNoneMatch: etag}}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	c.Assert(resp2.ContentLength(), chk.Equals, int64(len(blockBlobDefaultData)))
}
...
func (s *aztestsSuite) TestBlobDeleteNonExistant(c *chk.C) {
	...
	_, err := blobURL.Delete(ctx, DeleteSnapshotsOptionInclude, BlobAccessConditions{}, nil, nil, nil)
	validateStorageError(c, err, ServiceCodeBlobNotFound)
}
...
func (s *aztestsSuite) TestBlobDeleteSnapshot(c *chk.C) {
	...
	_, err = snapshotURL.Delete(ctx, DeleteSnapshotsOptionNone, BlobAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobDeleteSnapshotsOnly(c *chk.C) {
	...
	_, err := blobURL.CreateSnapshot(ctx, nil, BlobAccessConditions{})
	c.Assert(err, chk.IsNil)
	_, err = blobURL.Delete(ctx, DeleteSnapshotsOptionOnly, BlobAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func validateBlobDeleted(c *chk.C, blobURL BlockBlobURL) {
	_, err := blobURL.GetProperties(ctx, BlobAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.NotNil)
...
func (s *aztestsSuite) TestBlobDeleteIfModifiedSinceTrue(c *chk.C) {
	...
	_, err := blobURL.Delete(ctx, DeleteSnapshotsOptionNone,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfModifiedSince: currentTime}}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobDeleteIfUnmodifiedSinceTrue(c *chk.C) {
	...
	_, err := blobURL.Delete(ctx, DeleteSnapshotsOptionNone,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfUnmodifiedSince: currentTime}}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobDeleteIfMatchTrue(c *chk.C) {
	...
	resp, _ := blobURL.GetProperties(ctx, BlobAccessConditions{}, nil, nil, nil)
	etag := resp.ETag()

	_, err := blobURL.Delete(ctx, DeleteSnapshotsOptionNone,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfMatch: etag}}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobDeleteIfNoneMatchTrue(c *chk.C) {
	...
	_, err := blobURL.Delete(ctx, DeleteSnapshotsOptionNone,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfNoneMatch: etag}}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobGetPropsAndMetadataIfMatchTrue(c *chk.C) {
	...
	resp2, err := blobURL.GetProperties(ctx,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfMatch: resp.ETag()}}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	c.Assert(resp2.NewMetadata(), chk.DeepEquals, basicMetadata)
}
...
func (s *aztestsSuite) TestBlobGetPropsAndMetadataIfMatchFalse(c *chk.C) {
	...
	_, err := blobURL.GetProperties(ctx,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfMatch: ETag("garbage")}}, nil, nil, nil)
	c.Assert(err, chk.NotNil)
	serr := err.(StorageError)
	c.Assert(serr.Response().StatusCode, chk.Equals, 412)
}
...
func (s *aztestsSuite) TestBlobGetPropsAndMetadataIfNoneMatchTrue(c *chk.C) {
	...
	resp, err := blobURL.GetProperties(ctx,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfNoneMatch: ETag("garbage")}}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	c.Assert(resp.NewMetadata(), chk.DeepEquals, basicMetadata)
}
...
func (s *aztestsSuite) TestBlobGetPropsAndMetadataIfNoneMatchFalse(c *chk.C) {
	...
	_, err = blobURL.GetProperties(ctx,
		BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfNoneMatch: resp.ETag()}}, nil, nil, nil)
	c.Assert(err, chk.NotNil)
	serr := err.(StorageError)
...
func setAndCheckBlobTier(c *chk.C, containerURL ContainerURL, blobURL BlobURL, tier AccessTierType) {
	_, err := blobURL.SetTier(ctx, tier, LeaseAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobTierInferred(c *chk.C) {
	...
	_, err = blobURL.SetTier(ctx, AccessTierP4, LeaseAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobArchiveStatus(c *chk.C) {
	...
	_, err = blobURL.SetTier(ctx, AccessTierArchive, LeaseAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	_, err = blobURL.SetTier(ctx, AccessTierCool, LeaseAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	...
	_, err = blobURL.SetTier(ctx, AccessTierArchive, LeaseAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
	_, err = blobURL.SetTier(ctx, AccessTierHot, LeaseAccessConditions{}, nil, nil, nil)
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlobTierInvalidValue(c *chk.C) {
	...
	_, err = blobURL.SetTier(ctx, AccessTierType("garbage"), LeaseAccessConditions{}, nil, nil, nil)
...
func (s *aztestsSuite) TestDownloadBlockBlobUnexpectedEOF(c *chk.C) {
	...
	resp, err := bURL.Download(ctx, 0, 0, BlobAccessConditions{}, false, nil, nil, nil)
	c.Assert(err, chk.IsNil)
