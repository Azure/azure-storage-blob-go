...
func (s *aztestsSuite) TestGetBlobPropertiesUsingVID(c *chk.C) {
	...
	createResp, err := blobURL.Create(ctx, BlobHTTPHeaders{}, basicMetadata, BlobAccessConditions{ModifiedAccessConditions: ModifiedAccessConditions{IfMatch: blobProp.ETag()}}, ClientProvidedKeyOptions{})
...
func (s *aztestsSuite) TestCreateAndDownloadBlobSpecialCharactersWithVID(c *chk.C) {
	...
	for i := 0; i < len(data); i++ {
		...
		resp, err := blobURL.Upload(ctx, strings.NewReader(string(data[i])), BlobHTTPHeaders{}, Metadata{}, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
		c.Assert(err, chk.IsNil)
		c.Assert(resp.VersionID(), chk.NotNil)
...
func (s *aztestsSuite) TestDeleteSpecificBlobVersion(c *chk.C) {
	...
	blockBlobUploadResp, err := blobURL.Upload(ctx, bytes.NewReader([]byte("data")), BlobHTTPHeaders{},
		basicMetadata, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
...
func (s *aztestsSuite) TestCreateBlockBlobReturnsVID(c *chk.C) {
	...
	uploadResp, err := blobURL.Upload(ctx, r, BlobHTTPHeaders{}, Metadata{}, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestPutBlockListReturnsVID(c *chk.C) {
	...
	commitResp, err := blobURL.CommitBlockList(ctx, base64BlockIDs, BlobHTTPHeaders{}, Metadata{}, BlobAccessConditions{}, DefaultAccessTier, ClientProvidedKeyOptions{})
	c.Assert(err, chk.IsNil)
