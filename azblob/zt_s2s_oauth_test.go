...
func (s *aztestsSuite) TestBlockBlobS2SOAuth(c *chk.C) {
	...
	_, err = sourceBlob.Upload(ctx, strings.NewReader("Hello, World!"), BlobHTTPHeaders{}, nil, BlobAccessConditions{}, AccessTierHot, nil, ClientProvidedKeyOptions{})
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestBlockBlobS2SOAuthCopyFromURL(c *chk.C) {
	...
	_, err = destBlob.CopyFromURL(ctx, sourceBlob.URL(), nil, ModifiedAccessConditions{}, BlobAccessConditions{}, nil, AccessTierHot, nil, *ocred)
	c.Assert(err, chk.IsNil)
}
...
func (s *aztestsSuite) TestPageBlobS2SOAuth(c *chk.C) {
	...
	_, err = sourceBlob.Create(ctx, 512, 0, BlobHTTPHeaders{}, nil, BlobAccessConditions{}, PremiumPageBlobAccessTierNone, nil, ClientProvidedKeyOptions{})
	c.Assert(err, chk.IsNil)
	...
	_, err = destBlob.Create(ctx, 512, 0, BlobHTTPHeaders{}, nil, BlobAccessConditions{}, PremiumPageBlobAccessTierNone, nil, ClientProvidedKeyOptions{})
	c.Assert(err, chk.IsNil)
...
func (s *aztestsSuite) TestAppendBlobS2SOAuth(c *chk.C) {
	...
	_, err = sourceBlob.Create(ctx, BlobHTTPHeaders{}, nil, BlobAccessConditions{}, nil, ClientProvidedKeyOptions{})
	c.Assert(err, chk.IsNil)
	...
	_, err = destBlob.Create(ctx, BlobHTTPHeaders{}, nil, BlobAccessConditions{}, nil, ClientProvidedKeyOptions{})
	c.Assert(err, chk.IsNil)
